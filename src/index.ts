...
function validateCount(li: Element, cars?: number, carsNew?: number) {
    if (cars && carsNew) {
...
function getSubtype(value: string) {
    switch (value) {
        case 'Легковая':
            return 1;
        case 'Грузовая':
...
function addItem(li: Element, listCont: Element, options?: IItemOptions) {
    ...
    buttonDelete.addEventListener('click', async () => {
        ...
        if (itemTypeSelect.value === 'Автомобиль') {
            ...
            (card.querySelector('.cars-last-count')! as HTMLElement).innerText = card.getAttribute('cars-last-count')!;
        } else {
            card.setAttribute('cargos-last-count', (+card.getAttribute('cargos-last-count')! + 1).toString());
            (card.querySelector('.cargos-last-count')! as HTMLElement).innerText = card.getAttribute('cargos-last-count')!;
        }
    ...
    buttonApprove.addEventListener('click', async () => {
        ...
        if (listItem.getAttribute('edited') === 'true') {
            ...
            itemSubtypeP.innerText = itemSubtypeSelect.options[itemSubtypeSelect.selectedIndex]?.text;
...
async function createListElement(options: ICard) {
    ...
    buttonApprove.addEventListener('click', async () => {
        if (info.getAttribute('edited') === 'true') {
            ...
            items.forEach(item => {
                if (item.classList.contains('hidden')) sum_cargos += 1;
                else {
            ...
            (li.querySelector('.cargos-last-count')! as HTMLElement).innerText = String(new_cargos_count - sum_cargos);
            (li.querySelector('.cars-last-count')! as HTMLElement).innerText = String(new_cars_count - sum_cars);
    ...
    (li.querySelector('.cars-last-count')! as HTMLElement).innerText = li.getAttribute('cars-last-count')!;
...
async function addListElement() {
    ...
    if (modal) {
        ...
        async function createNewListElement() {
            ...
            const rows: IFerry[] = await result.json();
...
function enableDragAndDropCard(card: HTMLElement) {
    ...
    card.addEventListener('drop', async (e: DragEvent) => {
        ...
        const targetDestination = card.querySelectorAll<HTMLElement>('.trip-destination p')[1]?.innerText.trim();
        ...
        if (draggedItem && targetList && targetDestination === sourceDestination) {
            ...
            const itemTypeSelect = item.querySelector('.item-type select')! as HTMLSelectElement;
            const itemSubtypeSelect = item.querySelector('.item-subtype select')! as HTMLSelectElement;
            ...
            (card.querySelector('.cargos-last-count')! as HTMLElement).innerText = card.getAttribute('cargos-last-count')!;
            (card.querySelector('.cars-last-count')! as HTMLElement).innerText = card.getAttribute('cars-last-count')!;
            (sourceCard!.querySelector('.cargos-last-count')! as HTMLElement).innerText = sourceCard!.getAttribute('cargos-last-count')!;
            (sourceCard!.querySelector('.cars-last-count')! as HTMLElement).innerText = sourceCard!.getAttribute('cars-last-count')!;
        } else {
...
function enableDragAndDropListItem(item: HTMLElement) {
    ...
    item.addEventListener('dragstart', (e: DragEvent) => {
        ...
        const sourceDestination = sourceCard?.querySelectorAll<HTMLElement>('.trip-destination p')[1]?.innerText.trim();
